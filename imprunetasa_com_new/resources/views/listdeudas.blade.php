<table id="dt_basic" class="table table-striped table-bordered table-hover" width="100%">
	...
	<tbody>
		...
		<tr  style="background-color: #fff">
			...
			<td style="padding-top: 4px; padding-bottom: 4px; vertical-align: middle"><?php echo "$ "; ?>{{ number_format($deuda->importe_deuda,2)}}</td>
			<td style="padding-top: 4px; padding-bottom: 4px; vertical-align: middle">
				<a href="javascript:void(0);"  onclick="load_payment_promise({{$deuda->id_titular}},{{$deuda->id_deuda}}, '{{$deuda->dni_titular}}');" class="btn btn-xs btn-warning pull-right" data-toggle="modal" data-target="#myModal" rel="tooltip" data-placement="top" data-original-title="<h4>Realice un acuerdo de pago!</h4>" data-html="true" style="padding-top: 8px; padding-bottom: 2px; background-color: #0d0d0d" ><i class="fa fa-check fa-lg"></i> <label class="parpadea text"> Compromiso de pago</label></a>
			</td>
		</tr>
		<?php } ?>
	</tbody>
